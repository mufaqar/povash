 ...
 */
function povash_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'povash_unset_default_widgets_args' );
 ...
 */
function povash_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'povash_child_functions_php', 10, 2 );
 ...
 */
function povash_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'povash'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'povash_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'povash' ),
			'preview_url'              => 'http://commonsupport.net/newwp/povash/',
		),
	);
}

add_filter( 'merlin_import_files', 'povash_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function povash_after_import_setup() {
...
add_action( 'merlin_after_all_import', 'povash_after_import_setup' );