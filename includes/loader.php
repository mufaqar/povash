...
define( 'POVASH_ROOT', get_template_directory() . '/' );
...
add_action( 'after_setup_theme', 'povash_wp_load', 5 );

function povash_wp_load() {

	defined( 'POVASH_URL' ) or define( 'POVASH_URL', get_template_directory_uri() . '/' );
	define(  'POVASH_KEY','!@#povash');
	define(  'POVASH_URI', get_template_directory_uri() . '/');

	if ( ! defined( 'POVASH_NONCE' ) ) {
		define( 'POVASH_NONCE', 'povash_wp_theme' );
	}

	( new \POVASH\Includes\Classes\Base )->loadDefaults();
	( new \POVASH\Includes\Classes\Ajax )->actions();

}
add_action( 'init', 'povash_bunch_theme_init');
function povash_bunch_theme_init()
{
