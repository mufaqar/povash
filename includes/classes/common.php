...
case 'product': 
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => povash_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => povash_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => povash_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => povash_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => povash_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => povash_meta( 'banner_elementor_template', $page_id ),
				'banner'            => povash_set( povash_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => povash_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => povash_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : povash_the_title( $template ),
				'banner'        => povash_set( $bg, 'url' ),
